  ...
  */
  get : function(bucket, key, cb){
    contract(arguments)
        ...
        .end();
    this.db.collection(this.prefix + collectionName,function(err,collection){
      if(err instanceof Error) return cb(err);
      // Excluding bucket field from search result
      collection.findOne({bucket: bucket, key:key}, {bucket: 0},function(err, doc){
  ...
  */
  union : function(bucket, keys, cb){
    ...
    this.db.collection(this.prefix + collectionName,function(err,collection){
      if(err instanceof Error) return cb(err);
      // Excluding bucket field from search result
      collection.find({bucket: bucket, key: { $in: keys }}, {bucket: 0}).toArray(function(err,docs){
        if(err instanceof Error) return cb(err);
  ...
  */
  add : function(transaction, bucket, key, values){
    ...
    transaction.push(function(cb){
      ...
      self.db.collection(self.prefix + collectionName, function(err,collection){
        if(err instanceof Error) return cb(err);
        ...
        // update document
        collection.update({bucket: bucket, key:key},{$set:doc},{safe:true,upsert:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
  ...
  */
  del : function(transaction, bucket, keys){
    ...
    transaction.push(function(cb){
      self.db.collection(self.prefix + collectionName,function(err,collection){
        if(err instanceof Error) return cb(err);
        collection.remove({bucket: bucket, key:{$in:keys}},{safe:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
  ...
  remove : function(transaction, bucket, key, values){
    ...
    transaction.push(function(cb){
      self.db.collection(self.prefix + collectionName,function(err,collection){
        ...
        // update document
        collection.update({name:bucket, key:key},{$unset:doc},{safe:true,upsert:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
