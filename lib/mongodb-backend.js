  ...
  */
  get : function(bucket, key, cb){
    contract(arguments)
        ...
        .end();
    this.db.collection(this.prefix + aclCollectionName,function(err,collection){
      if(err instanceof Error) return cb(err);
      // Excluding bucket field from search result
      collection.findOne({name: bucket, key:key}, {name: 0},function(err, doc){
  ...
  */
  union : function(bucket, keys, cb){
    ...
    this.db.collection(this.prefix + aclCollectionName,function(err,collection){
      if(err instanceof Error) return cb(err);
      // Excluding bucket field from search result
      collection.find({name: bucket, key: { $in: keys }}, {name: 0}).toArray(function(err,docs){
        if(err instanceof Error) return cb(err);
  ...
  */
  add : function(transaction, bucket, key, values){
    ...
    transaction.push(function(cb){
      ...
      self.db.collection(self.prefix + aclCollectionName, function(err,collection){
        if(err instanceof Error) return cb(err);
        ...
        // update document
        collection.update({name: bucket, key:key},{$set:doc},{safe:true,upsert:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
  ...
  */
  del : function(transaction, bucket, keys){
    ...
    transaction.push(function(cb){
      self.db.collection(self.prefix + aclCollectionName,function(err,collection){
        if(err instanceof Error) return cb(err);
        collection.remove({name: bucket, key:{$in:keys}},{safe:true},function(err){
          if(err instanceof Error) return cb(err);
          cb(undefined);
        });
      });
