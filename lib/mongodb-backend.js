...
MongoDBBackend.prototype = {
  ...
  */
    ...
    self.transactions.find().toArray(function(err,docs){
      ...
      if(docs.length){
        return setTimeout(function(){self.end(transaction,cb);},0);
      }
  ...
  */
  get : function(bucket, key, cb){
    ...
    self.transactions.find().toArray(function(err,docs){
      // if a transaction is in progress, wait  
      if(docs.length){
        console.log("transaction in progress, deferring...");
        return setTimeout(function(){self.get(bucket, key, cb);},0);  
      }        
  ...
  union : function(bucket, keys, cb){
    ...
    self.transactions.find().toArray(function(err,docs){
      ...
      if(docs.length){
        console.log("transaction in progress, deferring...");
        return setTimeout(function(){self.union(bucket, keys, cb);},0);  
      }        
      // otherwise, find the document        
