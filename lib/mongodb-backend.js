...
MongoDBBackend.prototype = {
  ...
  */
  get : function(bucket, key, cb){
    contract(arguments)
        ...
        .end();
    var searchParams = (this.useSingle? {name: bucket, key:key} : {key:key});
    ...
    this.db.collection(this.prefix + collName,function(err,collection){
      ...
      collection.findOne(searchParams, {name: 0},function(err, doc){
  ...
  */
  union : function(bucket, keys, cb){
    ...
    var searchParams = (this.useSingle? {name: bucket, key: { $in: keys }} : {key: { $in: keys }});
    var collName = (this.useSingle? aclCollectionName : bucket);

    this.db.collection(this.prefix + collName,function(err,collection){
      if(err instanceof Error) return cb(err);
      // Excluding bucket field from search result
      collection.find(searchParams, {name: 0}).toArray(function(err,docs){
        if(err instanceof Error) return cb(err);
  ...
  */
  add : function(transaction, bucket, key, values){
    ...
    var self=this;
    var updateParams = (self.useSingle? {name: bucket, key:key} : {key:key});
  ...
  */
  del : function(transaction, bucket, keys){
    ...
    keys = makeArray(keys);
    var self= this;
    var updateParams = (self.useSingle? {name: bucket, key:{$in:keys}} : {key:{$in:keys}});
    var collName = (self.useSingle? aclCollectionName : bucket);
