...
Acl.prototype.removeAllow = function(role, resources, permissions, callback){
  ...
  var resourceKey = acl.roleResourcesKey(role);
  ...
  resources.forEach(function(resource){
    ...
    if(permissions){
      ...
      multi.scard(key, function(err, card){
        if(card===0){
          acl.redis.srem(resourceKey, resource);
        }
      });
    }else{
      multi.del(key);
      multi.srem(resourceKey, resource);
    }
  });
...
Acl.prototype.middleware = function(numPathComponents, userId, actions){
  ...
  return function(req, res, next){
    ...
    logger?logger.debug('Requesting '+actions+' on '+resource+' by user '+_userId):null;
    ...
    acl.isAllowed(_userId, resource, actions, function(err, allowed){
      if (err){
        ...
        logger?logger.debug('Not allowed '+actions+' on '+resource+' by user '+_userId):null;
        acl.allowedPermissions(_userId, resource, function(err, obj){
          logger?logger.debug('Allowed permissions: '+util.inspect(obj)):null;
        });
        next(new HttpError(401,'Insufficient permissions to access resource'));
      }else{
        logger?logger.debug('Allowed '+actions+' on '+resource+' by user '+_userId):null;
        next();
      }
    });
  };
