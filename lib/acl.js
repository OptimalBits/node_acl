...
var Acl = function (backend, logger, resourceHandler){
  contract(arguments)
    .params('object')
  ...
  this.resourceHandler = resourceHandler || new Acl.inheritableResourceHandler();
...
Acl.prototype.isAllowed = function(userId, resource, permissions, cb){
  ...
  if (Array.isArray(resource)) {
    if (!_this.resourceHandler) {
      cb(new Error('resourceHandler is required to support inheritable resource.'));
    } else {
      return this.allowedPermissions(userId, resource).then(function(resourcePermissions) {
        return _this.resourceHandler.isAllowed(permissions, resourcePermissions);
      }).nodeify(cb);
    }
  } else {
