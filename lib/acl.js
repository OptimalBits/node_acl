...
/**
  isAnyResourceAllowed( userId, resources, permissions, function(err, allowed) )

  Checks if the given user is allowed to access anyone of the resources for the given
  permissions (note: it must fulfill all the permissions).
  ...
  @param {Function} Callback called wish the result.
*/
Acl.prototype.isAnyResourceAllowed = function(userId, resources, permissions, cb){
  contract(arguments)
  ...
  var resArray = makeArray(resources);
  
  return this.isAllowed(userId, resArray.pop(), permissions).then(function(allow) {
    if (allow === true) {
      return true;
    } else if (resArray.length == 0) {
      return false;
    } else {
      return _this.isAnyResourceAllowed(userId, resArray, permissions);
    }
  }).nodeify(cb);
};
...
Acl.prototype.middleware = function(numPathComponents, userId, actions) {
  ...
  return function(req, res, next){
    ...
    _resourceBuilder(req, res, function(err, resources) {
      ...
      acl.isAnyResourceAllowed(_userId, resources, actions, function(err, allowed){
        if (err){
