...
var Acl = function (backend, logger, inheritableResourceHandler){
  contract(arguments)
    .params('object')
  ...
  this.inheritableResourceHandler = inheritableResourceHandler || new Acl.inheritableResourceHandler();
  if (this.inheritableResourceHandler) {
    this.inheritableResourceHandler = bluebird.promisifyAll(this.inheritableResourceHandler);
  }
...
Acl.prototype.isAllowed = function(userId, resource, permissions, cb){
  contract(arguments)
    .params('string|number', 'string|array', 'string|array', 'function')
    .params('string|number', 'string|array', 'string|array')
    .end();

  var _this = this;
  
  if (Array.isArray(resource)) {
    if (!_this.inheritableResourceHandler) {
      cb(new Error('inheritableResourceHandler is required to support inheritable resource.'));
    } else {
      return this.allowedPermissions(userId, resource).then(function(resourcePermissions) {
        return _this.inheritableResourceHandler.isAllowedAsync(permissions, resourcePermissions);
      }).nodeify(cb);
    }
  } else {
...
/**
  ...
  @param {number|function} number of path for building resouce ID. Or callback function building resource ID (function signature: function(req, res, cb))
  @param {string|number|function} userId or callback function to obtain userId (function signature: function(req, res))
  @param {string|array} actions
*/
Acl.prototype.middleware = function(numPathComponents, userId, actions) {
  contract(arguments)
    .params()
    .params('number|function')
    .params('number|function','string|number|function')
    .params('number|function','string|number|function', 'string|array')
    .end();
  ...
  return function(req, res, next){
    ...
    // callback function for building resource(s)
    if(typeof numPathComponents === 'function'){
      _resourceBuilder = numPathComponents;
    } else if (typeof numPathComponents === 'number') {
      _resourceBuilder = acl.pathResourceBuilder(numPathComponents);
    } else {
      acl.logger?acl.logger.debug('Invalid numPathComponents setting: '+numPathComponents):null;
      next(new HttpError(500, 'Invalid API call'));
      return;
    }

    _resourceBuilder(req, res, function(err, resources) {
      ...
      acl.logger?acl.logger.debug('Requesting '+actions+' on '+resources+' by user '+_userId):null;

      acl.isAllowed(_userId, resources, actions, function(err, allowed){
        if (err){
          next(new Error('Error checking permissions to access resource'));
        }else if(allowed === false){
          acl.logger?acl.logger.debug('Not allowed '+actions+' on '+resources+' by user '+_userId):null;
          ...
          acl.logger?acl.logger.debug('Allowed '+actions+' on '+resources+' by user '+_userId):null;
          next();
        }
      });
    });
...
/**
  pathResourceBuilder(numPathComponents)
  
  Create a resource builder to create resource from request url. 

  @param {numPathComponents} number of path for building resouce ID
*/
Acl.prototype.pathResourceBuilder = function(numPathComponents) {
  return function (req, res, next) {
    var resources = [];
    ...
    if (!numPathComponents) {
      ...
      resources.push(url.split('/').slice(0,numPathComponents+1).join('/'));
    }
    next(undefined, resources);
  }
}
