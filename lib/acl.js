  ...
  if (Array.isArray(resource)) {
    return this.allowedPermissions(userId, resource).then(function(resourcePermissions) {
      ...
      _.forEach(resourcePermissions, function(resPermissions) {
        mergedPermissions = _.union(mergedPermissions, resPermissions);
      });
      ...
      return makeArray(permissions).filter(function(p){
...
/**
  areResourcesAllowed( userId, resources, permissions, function(err, allowed) )

  Checks if the given user is allowed to access the resources for the given
  permissions (note: Permission of all resources are combined as one; it must fulfill all the 
  permissions).

  @param {String|Number} User id.
  @param {String|Array} Hierachical resources to ask permissions for.
  @param {String|Array} asked permissions.
  @param {Function} Callback called wish the result.
*/
//Acl.prototype.areResourcesAllowed = function(userId, resources, permissions, cb){
//  contract(arguments)
//    .params('string|number', 'string|array', 'string|array', 'function')
//    .params('string|number', 'string|array', 'string|array')
//    .end();
//
//  return this.allowedPermissions(userId, resources).then(function(resourcePermissions) {
//   
//    var mergedPermissions = [];
//    _.forEach(resourcePermissions, function(resPermissions) {
//      mergedPermissions = _.union(mergedPermissions, resPermissions);
