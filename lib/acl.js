...
*/
Acl.prototype.middleware = function(numPathComponents, userId, actions, dynamicParams){
  contract(arguments)
    .params()
    .params('number')
    .params('number','string|number|function')
    .params('number','string|number|function', 'string|array')
    .params('number','string|number|function', 'string|array', 'array')
    .end();
  ...
  return function(req, res, next){
    ...
    if(!numPathComponents){
      ...
      if(numPathComponents < 0){
        resource = url.split('/').splice(Math.abs(numPathComponents) + 1).join('/');
      }else{
    ...
    // map params
    if(req.params && req.route && req.route.path && dynamicParams instanceof Array){
      var path = req.route.path;
      var params = path.split(':').join('').split('/').filter(Boolean);

      dynamicParams.forEach(function(dp){
        if(params.indexOf(dp) !== -1){
          resource = resource.replace(req.params[dp], '').split('/').filter(Boolean).join('/');
        }
      });
    }
